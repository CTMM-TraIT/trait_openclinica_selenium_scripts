...
REM ---------------------------------------------------
%JAVA_EXECUTABLE% -jar selenium-server-standalone-2.44.0.jar -trustAllSSLCertificates -avoidProxy -userExtensions user-extensions.js -htmlSuite "*firefox  C:\\Program Files (x86)\\Mozilla Firefox\\firefox.exe" https://www.openclinica.nl/ test-performance-suite.html Results.html 
...
findstr  /R "Duration.*[0-9].*" Results.html | gawk "{print \"%DATE%\t%TIME%\t\"$6,$17}" >> results-runs.txt
